...
function ResultsTable({ searchRef }) {
  const { columns, initialValues, clientInfo, getClientMutation, handleOnUpdated, DeleteModalRef, state, expand, setExpand } = Logic(searchRef)
  const intl = useIntl()
  const { isLoading } = getClientMutation

  return (
    <>
      {state.SearchClientResult.length > 0 && (
        <CollapsiblePanel title={intl.formatMessage({ id: 'SEARCHCLIENT.SEARCHRESULTS' })} expanded={expand} onClick={() => setExpand((prevState) => !prevState)}>
          <Box mt={4}>
            <AppTable rows={state.SearchClientResult} columns={columns} />
          </Box>
        </CollapsiblePanel>
      )}
      {isLoading ? (
