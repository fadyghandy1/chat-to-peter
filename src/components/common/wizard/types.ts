import { FormikProps, FormikValues, FormikConfig, WithFormikConfig } from 'formik'
...
export interface Step {
  ...
  beforePrev?: (values: FormikValues, formikBag: FormikProps<FormikValues>, currentStepIndex: number) => Promise<any>
  ...
  component: React.ComponentType<FormikProps<FormikValues> & { currentStepIndex: number } & { mode?: string } & { isPreview?: boolean }>
  isPreview?: boolean
}
...
export interface WizardProps extends FormikConfig<FormikValues> {
  ...
  // children?: ((props: RenderProps) => React.ReactNode) | React.ReactElement<RenderProps> | ((props: FormikProps<FormikValues>) => React.ReactNode)

  /** Should validate the form before moving to next step */
  validateOnNext?: boolean

  /** Wizard mode add,edit,peview or other*/
